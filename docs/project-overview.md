...
- Docked ImGui shell (menu/toolbar/Properties/Jobs/Console), measurement,
  phone-created annotation pins, clipping with DXF/CSV/PNG slice export, EDL,
  stereoscopic SBS, controller + ESP32 serial input.
...
move protocol, remote tap-to-measure, remote tap-to-annotate with persisted
labels). Two viewport streaming engines, switchable at runtime via
`preferredStream`:
...
Current milestone: medium-feature plan items #1-#5 are complete
(`parallel-indexer`, `multi-client-roles`, `camera-path-export`,
`cross-section-slice-export`, `annotations-from-phone`). Next planned work is
#6 multi-cloud scene.
