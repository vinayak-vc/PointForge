...
- **Decision**: Detect display DPI via `SDL_GetDisplayDPI` (96 dpi = 1.0x) for the default; apply with `ImGuiStyle::ScaleAllSizes` from a captured base style plus `io.FontGlobalScale`. Exposed as a persisted "UI Scale" slider.
