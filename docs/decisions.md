...
## 3D Gaussian Splatting (3DGS) Support & Multi-Cloud Coexistence (docs/gsplat_feature.md)
- **Decision**: Drafted an authoritative 8-phase technical implementation plan (`docs/gsplat_feature.md`) to introduce native `.splat` and 3DGS `.ply` parsing, per-frame GPU depth sorting, and anisotropic alpha-blended screen-space splatting.
- **Reason**: 3DGS produces continuous, photorealistic surface environments that complement surveying/LiDAR scans. By integrating 3DGS layers directly into the multi-cloud `SceneCloud` vector, users can render LiDAR geometry and photogrammetric Gaussian Splat backgrounds simultaneously.
- **Design Choices**: 
  - Render LiDAR point clouds via `PointRenderer` (writing depth to framebuffer) and Gaussian Splats via a dedicated `SplatRenderer` (depth-test ON, depth-write OFF) so LiDAR points correctly occlude splats when in front.
  - Bypass Eye-Dome Lighting (`EdlPass`) post-processing for 3DGS layers while retaining EDL for uncoloured LiDAR clouds.
  - Leverage desktop-side rendering so Web Remote WebRTC/JPEG streaming and Stereoscopic SBS work out of the box without requiring mobile-client GPU splatting.
